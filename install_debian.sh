...
# This script will install KILLER NODES on a fresh Ubuntu/Debian server
...
check_requirements() {
    ...
    if ! [ -f /etc/os-release ]; then
        error "Cannot determine OS version. Only Ubuntu/Debian supported."
        exit 1
    fi
    
    . /etc/os-release
    if [[ "$NAME" != "Ubuntu" ]] && [[ "$NAME" != "Debian GNU/Linux" ]]; then
        error "Only Ubuntu/Debian is supported. Current OS: $NAME"
        exit 1
    fi
...
install_dependencies() {
    ...
    if [ -f /etc/os-release ]; then
        ...
        if [[ "$NAME" == "Ubuntu" ]]; then
            ...
            # For Debian, use available PHP version
            if apt list --installed 2>/dev/null | grep -q "php8.2"; then
                PHP_VERSION="php8.2"
            elif apt list --installed 2>/dev/null | grep -q "php8.1"; then
                PHP_VERSION="php8.1"
            else
                PHP_VERSION="php8.2"  # Default for Debian
            fi
        fi
    fi
...
start_services() {
    ...
    # Run initial setup (if the command exists)
    if [ -f "killernodes" ]; then
        log "Running initial setup..."
        php killernodes migrate 2>/dev/null || echo "Migration command not found, continuing..."
        php killernodes makeAdmin 2>/dev/null || echo "makeAdmin command not found, continuing..."
    else
        warn "killernodes command not found, skipping initial setup"
    fi
