...
# Default target
help:
	@echo -e "${BOLD}${BLUE}KILLER NODES Development System${NC}"
	@echo -e "${CYAN}================================${NC}\n"
	...
	@echo -e "  ${GREEN}make upgrade-core${NC} ${UPGRADE} Upgrades the core of KILLER NODES"
	@echo -e "  ${GREEN}make release-package${NC} ${PACKAGE} Creates a release package KILLER NODES.zip"
...
clean-license:
	...
	@rm -rf /var/www/killernodes-v3/backend/storage/caches/licenses/*.json 
...
release-package:
	...
	@$(MAKE) set-prod
	@$(MAKE) release
	@echo -e "${GREEN}${INFO} Creating KILLER NODES.zip package...${NC}"
	@echo -e "${YELLOW}${WARN} Creating package with tracked files and frontend dist...${NC}"
	@rm -f KILLER_NODES.zip
	@git ls-files | zip KILLER_NODES.zip -@
	@zip -r KILLER_NODES.zip $(FRONTEND_DIR)/dist
	@echo -e "${GREEN}${CHECK} Release package created: KILLER_NODES.zip${NC}\n"
...
get-tools: 
	...
	@echo -e "${BOLD}${GREEN}🎉 Tools are installed and you are ready to build KILLER NODES! 🎉${NC}\n"

# Upgrade KILLER NODES
upgrade-core:
	@echo -e "\n${BOLD}${BLUE}KILLER NODES Upgrade Process${NC} ${ROCKET}"
	@echo -e "${CYAN}============================${NC}"
	@echo -e "${YELLOW}${WARN} This will upgrade KILLER NODES to the latest version!${NC}"
	...
	@mariadb-dump -p killernodes_remastered > killernodes_backup.sql && echo -e "${GREEN}${CHECK} Database backup created: killernodes_backup.sql${NC}" || echo -e "${RED}${CROSS} Failed to create database backup${NC}"
	@cd /var/www && zip -r killernodes_backup.zip killernodes-v3/ && echo -e "${GREEN}${CHECK} File system backup created: /var/www/killernodes_backup.zip${NC}" || echo -e "${RED}${CROSS} Failed to create file system backup${NC}"
	@echo -e "\n${GREEN}${INFO} Step 2: Downloading latest release...${NC}"
	@curl -Lo KILLER_NODES.zip https://github.com/KillerNodes/KillerNodes/releases/latest/download/KILLER_NODES.zip && echo -e "${GREEN}${CHECK} Latest release downloaded${NC}" || (echo -e "${RED}${CROSS} Failed to download latest release${NC}" && exit 1)
	@echo -e "\n${GREEN}${INFO} Step 3: Extracting update files...${NC}"
	@unzip -o KILLER_NODES.zip -d /var/www/killernodes-v3 && echo -e "${GREEN}${CHECK} Files extracted successfully${NC}" || (echo -e "${RED}${CROSS} Failed to extract files${NC}" && exit 1)
	@rm -f KILLER_NODES.zip && echo -e "${GREEN}${CHECK} Cleanup: Removed zip file${NC}"
	...
	@php killernodes migrate && echo -e "${GREEN}${CHECK} Database migrations completed${NC}" || echo -e "${RED}${CROSS} Database migrations failed${NC}"
	@echo -e "\n${GREEN}${INFO} Step 6: Setting proper file permissions...${NC}"
	@chown -R www-data:www-data /var/www/killernodes-v3/* && echo -e "${GREEN}${CHECK} File permissions set${NC}" || echo -e "${YELLOW}${WARN} Could not set file permissions (you may need to run as root)${NC}"
	@echo -e "\n${GREEN}${INFO} Step 7: Setting production mode...${NC}"
	@$(MAKE) set-prod
	@echo -e "\n${BOLD}${GREEN}🎉 Upgrade completed successfully! 🎉${NC}"
	@echo -e "${GREEN}${CHECK} KILLER NODES has been upgraded to the latest version${NC}"
	@echo -e "${CYAN}${INFO} Backup files created:${NC}"
	@echo -e "  - Database: killernodes_backup.sql"
	@echo -e "  - Files: /var/www/killernodes_backup.zip\n"
